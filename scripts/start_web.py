...
def _kill_port_holders_win(ports: list[int]) -> None:
    """Windows-only: force-kill every process LISTENING on the given ports.

    Uses netstat -ano directly so it catches orphaned webpack-worker grandchildren
    that survive ``taskkill /T`` (which only walks one level of the process tree).
    """
...
def _cleanup_previous_launch_if_safe(ports: dict[str, int], language: str) -> None:
    state = _read_state()
    if state is None:
        # No state file — still purge any orphaned processes squatting on our ports
        # (covers sessions that crashed before writing state, or whose PIDs are gone).
    ...
    if conflicts and (
        ...
        _terminate_state_processes(state)
        _remove_state()
    elif not conflicts and _state_has_live_process(state):
        log_warn(_t(language, "cleanup_previous"))
        _terminate_state_processes(state)
        _remove_state()
    elif not conflicts and not _state_has_live_process(state):
        _remove_state()
...
def main() -> None:
    ...
    if not env_local_path.exists() or env_local_path.read_text(encoding="utf-8") != new_env_local:
        env_local_path.write_text(new_env_local, encoding="utf-8")
