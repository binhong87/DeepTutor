  ...
  return (
    <div className="max-w-5xl mx-auto px-6 py-8">
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-2xl font-semibold text-[var(--foreground)]">{t("Your TutorBots")}</h1>
          <p className="mt-1 text-sm text-[var(--muted-foreground)]">
            {isAuthenticated && status?.username
              ? t("Logged in as {{username}}", { username: status.username })
              : t("Manage your AI tutoring agents")}
          </p>
        </div>
        <Link
      ...
      {bots.length === 0 ? (
        <div className="rounded-xl border border-dashed border-[var(--border)] p-12 text-center">
          ...
          <h2 className="mt-4 text-lg font-medium text-[var(--foreground)]">{t("No TutorBots yet")}</h2>
          <p className="mt-1 text-sm text-[var(--muted-foreground)]">
            {t("Create your first TutorBot to get started with personalized AI tutoring.")}
          </p>
          <Link
            href="/tutorbot/boarding"
